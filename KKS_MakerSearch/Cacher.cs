...
namespace KKS_MakerSearch
{
    public static class Cacher
    {
        ...
        {
            if (!File.Exists(KKS_MakerSearch.TranslationCachePath))
                return;

            try
            {
                UnityEngine.Debug.Log("Loading MakerSearch cache from " + KKS_MakerSearch.TranslationCachePath);

                var cacheBytes = File.ReadAllBytes(KKS_MakerSearch.TranslationCachePath);
        ...
        private static void WriteCache()
        {
            try
            {
                UnityEngine.Debug.Log("Writing MakerSearch cache to " + KKS_MakerSearch.TranslationCachePath);

                var data = MessagePackSerializer.Serialize(TranslationLookup);
                File.WriteAllBytes(KKS_MakerSearch.TranslationCachePath, data);
            }
            catch (Exception e)
            {
